...
```ts
import type { TlsConfig } from '@stacksjs/dnsx'
import { startProxy } from '@stacksjs/dnsx'
...
```bash
dnsx --from localhost:3000 --to my-project.localhost
dnsx --from localhost:8080 --to my-project.test --keyPath ./key.pem --certPath ./cert.pem
dnsx --help
dnsx --version
```
